...
class AppFunctions(MainWindow):
    ...
    def connect2device(self):
        ...
        if self.is_connected is False:
            ...
            try:
                ...
                AppFunctions._stop_processes(self)
                AppFunctions._reset_vars(self)
    ...
    def plot_exg(self, data):
        ...
        exg_plot_nan = copy.deepcopy(self.exg_plot)
        # print("self.exg_plot: ", np.count_nonzero(np.isnan(self.exg_plot["ch1"])))
        # print("exg_plot_nan: ", np.count_nonzero(np.isnan(exg_plot_nan["ch1"])))

        for ch in exg_plot_nan.keys():
            # print(ch, " ", np.count_nonzero(np.isnan(exg_plot_nan[ch])), "\n")
            exg_plot_nan[ch][self.exg_pointer-1:self.exg_pointer+9] = np.NaN
            # if ch == "ch1":
            #     print("AFTER exg_plot_nan: ", np.count_nonzero(np.isnan(exg_plot_nan[ch])), "\n")


        # Paint curves
        for curve, ch in zip(self.curves_list, self.active_chan):
            # curve.setData(self.t_exg_plot, self.exg_plot[ch], connect=connection)
            curve.setData(self.t_exg_plot, exg_plot_nan[ch], connect="finite")
